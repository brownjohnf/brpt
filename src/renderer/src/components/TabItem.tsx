...
export function TabItem({ tab, isActive, groupRootPath, onClick, onClose }: TabItemProps): JSX.Element {
  const filename = getFilename(tab.path)
  const prefix = groupRootPath ? getRelativePrefix(tab.path, groupRootPath) : null
    ...
    >
      ...
      >
        <bdo dir="ltr">
          ...
          <span style={groupRootPath ? { color: textColor } : undefined}>{filename}</span>
        </bdo>
      </span>
      <button
        className={classNames(
